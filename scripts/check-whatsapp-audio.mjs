...
const tempDir = path.join(rootDir, "tmp-whatsapp-audio-check");
...
const source = await readFile(path.join(rootDir, "tests", "whatsapp-audio.test.mjs"), "utf8");
const transformed = source
  .replace('import assert from "node:assert/strict";', 'const assert = require("node:assert/strict");')
  .replace('import test from "node:test";', 'const test = require("node:test");')
  .replace(/import\s+\{([\s\S]*?)\}\s+from\s+"([^"]+)";/g, (_match, imports, specifier) => {
    const names = imports
      .split(",")
      .map((name) => name.trim())
      .filter(Boolean)
      .join(", ");
    const target = specifier.replace(/\.ts$/, ".js");

    return `const { ${names} } = require("${target}");`;
  });

await mkdir(path.join(distDir, "tests"), { recursive: true });
const testFile = path.join(distDir, "tests", "whatsapp-audio.test.cjs");
await writeFile(testFile, transformed);

const testResult = spawnSync(process.execPath, [testFile], {
  cwd: rootDir,
  stdio: "inherit",
});
